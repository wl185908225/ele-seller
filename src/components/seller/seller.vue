...
<script type="text/javascript">
  ...
  export default {
    ...
    mounted() {
        ...
        setTimeout(function() {
            if (!that.scroll) {
                that.scroll = new BScroll(that.$refs.seller, {
                    click: true,
                    mouseWheel: true
                });
            } else {
            ...
            if (that.seller.pics) {
                let picWidth = 120;
                let margin = 6;
                let width = (picWidth + margin) * that.seller.pics.length - margin;
            ...
            that.favorite = loadFromLocal(that.seller.id, 'favorite', false);
        }, 200);
    },
    methods: {
        toggleFavorite() {
